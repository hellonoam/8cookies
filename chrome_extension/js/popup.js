...
function userLoggedIn(username, paused) {
    $(".whenNotLoggedIn").hide();
    $(".loggedInUser").show();
    $(".loggedInUser p:first").text("hello " + username);
    if (paused && paused != "false")
        $(".togglePause").val("resume sync");
}
...
function init() {
    chrome.extension.sendRequest(
        ...
        function(response) {
            ...
            if (response.result && response.result != "false")
                userLoggedIn(response.username, response.paused);
            else
                userLoggedOut();
        }
    );
    ...
    setInterval(function() {
        if ($(".togglePause").val().indexOf("pause") != -1)
            $(".syncText").text("synced " + 
                chrome.extension.getBackgroundPage().background.user.elapsedSinceLastSync() +
                 " ago...");
        else
            $(".syncText").text("sync paused");
    }, 1000);
    ...
    $(".togglePause").click(function() {
        chrome.extension.sendRequest({ type: "togglePause"});
        if ($(".togglePause").val().indexOf("pause") != -1)
            $(".togglePause").val("resume sync");
        else
            $(".togglePause").val("pause sync");
    });
